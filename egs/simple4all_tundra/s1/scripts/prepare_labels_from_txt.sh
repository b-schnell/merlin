...
if [ ! -d "${FRONTEND_DIR}" ]; then
    echo "Please configure FRONTEND_DIR path in $global_config_file !!"
    exit 1
fi
...
${FRONTEND_DIR}/makeLabels.sh ${FESTDIR} ${inp_txt}/utts.data ${out_dir} \
               2>&1 \
               1>$out_dir/makeLabels.log | tee -a $out_dir/makeLabels.log
# Rename folder labels created by frontend to prompt-lab which is normally used by merlin.
rm -rf $out_dir/prompt-lab # Remove old files.
mv $out_dir/labels $out_dir/prompt-lab
